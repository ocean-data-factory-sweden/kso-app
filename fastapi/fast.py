...
class KosterModel:
    def __init__(self):
        with torch.no_grad():
            ...
            # Initialize model
            self.model = Darknet("cfg/yolov3-spp-4cls-416.cfg", self.img_size)
    ...
    async def detect(self, save_img=False):
        boxes = []
        vid = False
        detect_dict = {}
        ...
        with torch.no_grad():
            ...
            if vid:
                ...
                return {"result": tuple([nvid_path, vid, detect_dict])}
            #open(nvid_path, "rb").read(), vid, detect_dict
                ...
                return {"result": tuple([resized, vid, detect_dict])}
...
async def predict(media_path: str, conf_thres: float, iou_thres: float):
    ...
    pred, vid, detect_dict = result["result"]
    if vid:
        pred = list(pred)
    else:
        pred = pred.tolist()
